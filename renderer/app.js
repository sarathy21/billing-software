...
async function beginReturnEntry(type) {
  ...
  const title = document.getElementById('transactionPartyTitle');
  if (title) {
    title.textContent = currentTransactionType === 'sales_return'
      ? 'Sales Return - Party List'
      : 'Purchase Return - Party List';
  }
...
function togglePaymentOtherMode(prefix) {
  ...
  if (!modeSelect || !otherInput) return;
...
function startEditPayment(id) {
  ...
  const knownModes = ['Cash', 'UPI', 'Bank Transaction', 'Cheque'];
...
function onRawMaterialEntryTypeChange() {
  ...
  if (isOut) {
    ...
    const detailsInput = document.getElementById('rawMaterialProductDetails');
    if (rateInput) {
      rateInput.value = '';
    }
    if (detailsInput) {
...
function getRawMaterialFormPayload() {
  ...
  return {
    ...
    product_details: isOut ? '' : String(document.getElementById('rawMaterialProductDetails')?.value || '').trim(),
  ...
  rawMaterialStockRows.forEach((row) => {
    ...
    html += `
      <tr class="border-t">
        ...
        <td class="p-2">
          <button onclick="editRawMaterialStockProduct('${escapeHtml(row.product_name)}')" class="bg-amber-500 text-white px-3 py-1 rounded mr-2">Edit</button>
...
async function removeRawMaterialStockProduct(productName) {
  if (!window.confirm(`Delete ALL transactions for product "${productName}"? This cannot be undone.`)) {
    return;
  }
  ...
  rawMaterialTransactions.forEach((row) => {
    ...
    html += `
      <tr class="border-t">
        ...
        <td class="p-2">${escapeHtml(detailsLabel)}</td>
        <td class="p-2">
  ...
  rows.forEach((row) => {
    ...
    html += `
      <tr class="border-t ${isLatest ? 'bg-amber-50' : ''}">
        ...
        <td class="p-2 text-right">${formatCurrency(row.packing_charge || 0)}</td>
        <td class="p-2 text-right">${formatCurrency(row.transport_charge || 0)}</td>
